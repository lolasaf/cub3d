...
void	handle_sprites(my_game *g)
{
	...
	{
		to_sort[i].dist = ((game->player_x - game->sprites[i].x)
				* (game->player_x - game->sprites[i].x)) +
																	((game->player_y
																			- game->sprites[i].y)
																		* (game->player_y
																			- game->sprites[i].y));
...
void	get_horizonatal_prop(t_sprite_props *prop)
{
	...
	// Often same as height for square sprites could also be tx
...
unsigned int	get_sprite_pixel_color(t_img *sprite_tex, int tex_x, int tex_y)
{
	...
	// Ensure tex_x and tex_y are within bounds (already done in calling functions,
		...
		t_sprite_props *prop)
{
	...
	stripe_x = prop->start_x;
	while (stripe_x < prop->end_x)
	{
		if (stripe_x >= 0 && stripe_x < SCREEN_WIDTH
			&& prop->t_y < g->z_buffer[stripe_x])
		{
			tex_x = (int)(256 * (stripe_x - (-prop->s_w / 2 + prop->s_sx))

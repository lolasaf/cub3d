...
/*   Updated: 2025/05/30 01:49:54 by wel-safa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_colorortext(char *str, char *token, t_data *data, char *frline)
{
	...
	if ((int)ft_strncmp(token, "NO", 2) == 0
		|| (int)ft_strncmp(token, "SO", 2) == 0
		|| (int)ft_strncmp(token, "WE", 2) == 0
		|| (int)ft_strncmp(token, "EA", 2) == 0)
	{
...
int	ft_parse_tandc_skip_spaces(const char **p, const char	**token_start, 
	int *token_len)
{
...
void	ft_parse_tandc(const char *line, t_data *data)
{
	...
	while (*p)
	{
		if (!ft_parse_tandc_skip_spaces(&p, &token_start, &token_len))
			break ;

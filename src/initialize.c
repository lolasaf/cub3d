...
/*   Updated: 2025/05/25 02:21:52 by wel-safa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize_game(my_game *game, t_data *data)
{
	...
	if (!game->z_buffer)
		err_msg("Failed to allocate z buff");
	game->mlx = mlx_init();
	if (!game->mlx)
	...
	if (!game->img->img_ptr)
		err_msg("Failed to create main img");
	(game->img)->addr = mlx_get_data_addr(game->img->img_ptr, 
			&game->img->bbp, &game->img->line_length, &game->img->endian);
	if (!game->img->addr)
		err_msg("Failed to get address");
